...
export default function Deliver() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      const queryUpdate = {
        ...query,
        dateDeliver: data.data.dateDeliver,
        deliverDetail: JSON.stringify(data.data.deliverDetail),
        signatureSupport: data.data.signatureSupport
      }
      //Update Query Params
  ...
  useEffect(() => {
    if (Object.keys(query).length > 0) {
      form.reset({
        deliverDetail: query.deliverDetail ? JSON.parse(query.deliverDetail as string) : [],
        dateDeliver: query.dateDeliver ? new Date(query.dateDeliver as string) : new Date(),
        signatureSupport: (query.signatureSupport as string),
      });
    }
  }, [query, form]);
                ...
                <div className={`flex justify-between mt-4`}>
                  ...
                  <div className="flex space-x-4">
                    <Button
                      ...
                      onClick={() => router.push({ pathname: "train", query: query })}
                    >
                      Back
                    </Button>
                    <Button
